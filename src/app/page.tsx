...
function MiniTetrisGame() {
  ...
  const canPlaceTetromino = useCallback((boardState: BoardType, tetromino: TetrominoType, row: number, col: number): boolean => {
      ...
      for (let c = 0; c < tetromino[r].length; c++) {
        if (tetromino[r][c]) {
          ...
          if (boardRow >= BOARD_ROWS || boardCol < 0 || boardCol >= BOARD_COLS || 
              (boardRow >= 0 && boardState[boardRow][boardCol] !== null)) {
            return false;
          }
        }
      }
  ...
  // Find the lowest valid position for a tetromino
  const findLowestValidPosition = useCallback((boardState: BoardType, tetromino: TetrominoType, col: number): number => {
    let row = 0;
    while (row < BOARD_ROWS && canPlaceTetromino(boardState, tetromino, row, col)) {
      row++;
    }
  ...
  const clearCompletedRows = useCallback((boardState: BoardType): { newBoard: BoardType; linesCleared: number } => {
    let linesCleared = 0;
  ...
  // Generate a new piece with only valid columns
  const generateNewPiece = useCallback((boardState: BoardType) => {
    const randomIdx = Math.floor(Math.random() * TETROMINOES.length);
    const tetromino = TETROMINOES[randomIdx].shape;
    const pieceWidth = tetromino[0].length;
    const validCols: number[] = [];
    for (let col = 0; col <= BOARD_COLS - pieceWidth; col++) {
      if (canPlaceTetromino(boardState, tetromino, 0, col)) {
        validCols.push(col);
      }
    }
    if (validCols.length === 0) {
      return null; // No valid spawn positions
    }
    const randomCol = validCols[Math.floor(Math.random() * validCols.length)];
    return { idx: randomIdx, col: randomCol };
  }, [canPlaceTetromino]);
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      setBoard(currentBoard => {
        // If no active piece, generate one
        if (!active) {
          const newPiece = generateNewPiece(currentBoard);
          if (!newPiece) {
            setGameOver(true);
            setIsPlaying(false);
            return currentBoard;
          }
          setActive({ idx: newPiece.idx, row: 0, col: newPiece.col });
          return currentBoard;
        }

        // Move active piece down
        const tetromino = TETROMINOES[active.idx].shape;
        const nextRow = active.row + 1;
        
        if (canPlaceTetromino(currentBoard, tetromino, nextRow, active.col)) {
          setActive({ ...active, row: nextRow });
          return currentBoard;
        } else {
          // Place the piece
          const newBoard = placeTetromino(currentBoard, tetromino, active.row, active.col, TETROMINOES[active.idx].color);
          
          // Clear completed rows
          const { newBoard: clearedBoard, linesCleared } = clearCompletedRows(newBoard);
          if (linesCleared > 0) {
  ...
  }, [isPlaying, gameOver, active, canPlaceTetromino, findLowestValidPosition, generateNewPiece, clearCompletedRows]);
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      setBoard(currentBoard => {
        ...
        switch (e.key) {
          case 'ArrowLeft':
            if (canPlaceTetromino(currentBoard, tetromino, active.row, active.col - 1)) {
              setActive({ ...active, col: active.col - 1 });
            }
            break;
          case 'ArrowRight':
            if (canPlaceTetromino(currentBoard, tetromino, active.row, active.col + 1)) {
            ...
            break;
          case 'ArrowDown':
            if (canPlaceTetromino(currentBoard, tetromino, active.row + 1, active.col)) {
  ...
  }, [isPlaying, active, canPlaceTetromino]);
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        {gameOver && (
          <div className="text-center">
            <div className="text-red-400 text-lg font-bold mb-2">Game Over!</div>
        ...
        {isPlaying && (
          <div className="text-center">
            <div className="text-green-400 text-lg font-bold">Score: {score}</div>
